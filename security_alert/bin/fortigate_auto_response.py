...
class FortiManagerAPI:
    ...
    def __init__(self, url: str, token: str):
        ...
        self.token = token
        self.verify_ssl = (
            os.environ.get("FORTIMANAGER_VERIFY_SSL", "true").lower() != "false"
        )
    ...
    def add_address_blacklist(self, ip_address: str, description: str) -> Dict:
        ...
        try:
            response = self.session.post(
                endpoint, json=payload, verify=self.verify_ssl, timeout=10
            )
...
class SlackNotifier:
    ...
    def send_notification(
        self, alert_name: str, action_taken: str, details: Dict
    ) -> bool:
    ...
    def _handle_brute_force(self, alert_data: Dict) -> Dict:
        ...
        if fail_count >= 10:
            ...
            result = self.fmg_api.add_address_blacklist(
                source_ip, f"Brute force attack: {fail_count} failed attempts"
            )
            return result
        else:
