...
class DeploymentHealthCheck:
            ...
            if 'count' in result.stdout:
                ...
                for line in lines:
                    if line.strip().isdigit():
                        ...
                        if count > 0:
                            check_result['details'].append(f"✅ index=fw 데이터 존재: {count} 이벤트 (최근 1시간)")
                        else:
                            check_result['status'] = 'WARNING'
                            check_result['details'].append("⚠️ index=fw 데이터 없음 (최근 1시간)")
                            self.results['warnings'].append("최근 1시간 동안 index=fw에 데이터 없음")
                        break
            else:
                check_result['status'] = 'WARNING'
    ...
    def check_slack_integration(self):
        ...
        if alert_actions_path.exists():
            ...
            if has_bot_token or has_webhook:
                ...
                check_result['status'] = 'WARNING'
                check_result['details'].append("⚠️ Slack 인증 미설정 (bot_token 또는 webhook_url 필요)")
    ...
    def check_lookup_health(self):
        ...
        if lookups_dir.exists():
            ...
            if empty_files:
                check_result['status'] = 'WARNING'
                check_result['details'].append(f"⚠️ 빈 룩업 파일: {', '.join(empty_files)}")
                self.results['warnings'].append(f"빈 룩업 파일 존재: {len(empty_files)}개")
        else:
            check_result['status'] = 'ERROR'
    ...
    def print_results(self):
        ...
        warning_checks = len([c for c in self.results['checks'] if c['status'] == 'WARNING'])
        error_checks = len([c for c in self.results['checks'] if c['status'] == 'ERROR'])
