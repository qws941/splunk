...
LOOKUP_FILE = os.path.join(
    os.path.dirname(__file__), "../lookups/virustotal_lookup.csv"
)
...
def check_file_hash(file_hash, api_key=None):
    ...
    try:
        ...
        if e.code == 404:
            ...
            print(
                f"ERROR: HTTP {e.code} for hash {file_hash}: {e.reason}",
                file=sys.stderr,
            )
        return None
...
def update_lookup_table(hash_data_list):
    ...
    try:
        ...
        print(
            f"SUCCESS: Updated {LOOKUP_FILE} with {len(existing_data)} hashes",
            file=sys.stderr,
        )
        return True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch VirusTotal file hash intelligence"
    )
    parser.add_argument(
        "--source",
    ...
    for i, file_hash in enumerate(hashes):
        ...
        if hash_data:
            ...
            print(
                f"  ✓ {file_hash}: {hash_data['threat_category']} ({hash_data['detection_rate']})",
                file=sys.stderr,
            )
        else:
