...
def load_whitelist():
    ...
    try:
        ...
        logger.info('', extra={'action': 'load_whitelist', 'ip': '-', 'score': '-', 'rule': '-', 'status': 'success', 'message': f'Loaded {len(whitelist)} whitelisted IPs'})
    except FileNotFoundError:
        logger.warning('', extra={'action': 'load_whitelist', 'ip': '-', 'score': '-', 'rule': '-', 'status': 'warning', 'message': 'Whitelist file not found - continuing without whitelist'})
    return whitelist
...
def load_blocked_ips():
    ...
    try:
        with open(BLOCKED_IPS_CSV, 'r') as f:
            ...
            for row in reader:
                blocked.add(row['ip'])
        logger.info('', extra={'action': 'load_blocked_ips', 'ip': '-', 'score': '-', 'rule': '-', 'status': 'success', 'message': f'Loaded {len(blocked)} already blocked IPs'})
    except FileNotFoundError:
        # File doesn't exist yet - will be created on first block
        logger.info('', extra={'action': 'load_blocked_ips', 'ip': '-', 'score': '-', 'rule': '-', 'status': 'info', 'message': 'Blocked IPs file not found - creating new tracking file'})
    return blocked
...
def save_blocked_ip(ip, correlation_score, correlation_rule, action_result):
    ...
    try:
        ...
        with open(BLOCKED_IPS_CSV, 'a') as f:
            writer = csv.DictWriter(f, fieldnames=['ip', 'blocked_at', 'correlation_score', 'correlation_rule', 'status'])
            if not file_exists:
                writer.writeheader()
            writer.writerow({
                'ip': ip,
        ...
        logger.info('', extra={'action': 'save_blocked_ip', 'ip': ip, 'score': correlation_score, 'rule': correlation_rule, 'status': 'success', 'message': 'Saved to blocked IPs tracking file'})
    except Exception as e:
        logger.error('', extra={'action': 'save_blocked_ip', 'ip': ip, 'score': correlation_score, 'rule': correlation_rule, 'status': 'error', 'message': str(e)})

def send_slack_notification(ip, correlation_score, correlation_rule, action_result, error_message=None):
    """Send Slack notification about block action."""
    if not SLACK_WEBHOOK_URL:
        return

    color = '#d93f3c' if action_result == 'blocked' else '#f58f39'
    title = f'🚫 Auto-Block: {ip}' if action_result == 'blocked' else f'⚠️ Block Failed: {ip}'
    ...
    payload = {
        'attachments': [{
            ...
            'fields': [
                ...
                {'title': 'Correlation Score', 'value': f'{correlation_score}/100', 'short': True},
                {'title': 'Correlation Rule', 'value': correlation_rule, 'short': True},
    ...
    try:
        ...
        if response.status_code == 200:
            logger.info('', extra={'action': 'slack_notify', 'ip': ip, 'score': correlation_score, 'rule': correlation_rule, 'status': 'success', 'message': 'Slack notification sent'})
    except Exception as e:
        logger.error('', extra={'action': 'slack_notify', 'ip': ip, 'score': correlation_score, 'rule': correlation_rule, 'status': 'error', 'message': str(e)})
...
class FortiGateAPIClient:
    ...
    def __init__(self, host, port, api_key, vdom='root'):
        ...
        if self.session:
            ...
            self.session.verify = os.environ.get('FORTIGATE_SSL_VERIFY', 'false').lower() == 'true'
    ...
    def create_address_object(self, ip):
        ...
        try:
            ...
            logger.info('', extra={'action': 'fortigate_create_address', 'ip': ip, 'score': '-', 'rule': '-', 'status': 'success', 'message': f'Created address object: {address_name}'})
            return address_name
        except Exception as e:
            logger.error('', extra={'action': 'fortigate_create_address', 'ip': ip, 'score': '-', 'rule': '-', 'status': 'error', 'message': str(e)})
            raise
    ...
    def create_deny_policy(self, address_name, ip):
        ...
        try:
            if self.session:
                response = self.session.post(url, params=params, json=payload, timeout=10)
                response.raise_for_status()
            logger.info('', extra={'action': 'fortigate_create_policy', 'ip': ip, 'score': '-', 'rule': '-', 'status': 'success', 'message': f'Created deny policy: {policy_name}'})
            return policy_name
        except Exception as e:
            logger.error('', extra={'action': 'fortigate_create_policy', 'ip': ip, 'score': '-', 'rule': '-', 'status': 'error', 'message': str(e)})
            raise
...
def process_correlation_results(results):
    ...
    if not FORTIGATE_API_KEY:
        logger.error('', extra={'action': 'init_api_client', 'ip': '-', 'score': '-', 'rule': '-', 'status': 'error', 'message': 'FORTIGATE_API_KEY not configured - auto-blocking disabled'})
        return

    fg_client = FortiGateAPIClient(FORTIGATE_HOST, FORTIGATE_PORT, FORTIGATE_API_KEY, FORTIGATE_VDOM)
    ...
    for result in results:
        ...
        if not src_ip:
            logger.warning('', extra={'action': 'validate_result', 'ip': '-', 'score': '-', 'rule': correlation_rule, 'status': 'warning', 'message': 'Missing src_ip field - skipping'})
            continue

        # Check whitelist
        if src_ip in whitelist:
            logger.info('', extra={'action': 'check_whitelist', 'ip': src_ip, 'score': correlation_score, 'rule': correlation_rule, 'status': 'skipped', 'message': 'IP is whitelisted - skipping auto-block'})
            continue

        # Check if already blocked
        if src_ip in already_blocked:
            logger.info('', extra={'action': 'check_blocked', 'ip': src_ip, 'score': correlation_score, 'rule': correlation_rule, 'status': 'skipped', 'message': 'IP already blocked - skipping duplicate'})
            continue

        # Check auto-block threshold
        if action_recommendation != 'AUTO_BLOCK' or correlation_score < AUTO_BLOCK_THRESHOLD:
            logger.info('', extra={'action': 'check_threshold', 'ip': src_ip, 'score': correlation_score, 'rule': correlation_rule, 'status': 'skipped', 'message': f'Score {correlation_score} below auto-block threshold {AUTO_BLOCK_THRESHOLD} - manual review required'})

            # Send Slack alert for review-required IPs
            if action_recommendation == 'REVIEW_AND_BLOCK':
                send_slack_notification(src_ip, correlation_score, correlation_rule, 'review_required')
            continue

        # Execute auto-block
        logger.info('', extra={'action': 'auto_block_start', 'ip': src_ip, 'score': correlation_score, 'rule': correlation_rule, 'status': 'processing', 'message': f'Starting auto-block (score: {correlation_score}, rule: {correlation_rule})'})

        block_result = fg_client.block_ip(src_ip)

        if block_result['status'] == 'success':
            logger.info('', extra={'action': 'auto_block_complete', 'ip': src_ip, 'score': correlation_score, 'rule': correlation_rule, 'status': 'success', 'message': f'Successfully blocked IP (address: {block_result["address"]}, policy: {block_result["policy"]})'})

            # Save to blocked IPs tracking
            save_blocked_ip(src_ip, correlation_score, correlation_rule, 'blocked')

            # Send success notification
            send_slack_notification(src_ip, correlation_score, correlation_rule, 'blocked')

        else:
            error_msg = block_result.get('error', 'Unknown error')
            logger.error('', extra={'action': 'auto_block_failed', 'ip': src_ip, 'score': correlation_score, 'rule': correlation_rule, 'status': 'error', 'message': error_msg})

            # Send failure notification
            send_slack_notification(src_ip, correlation_score, correlation_rule, 'failed', error_msg)
...
def main():
    ...
    try:
        ...
        if not results:
            logger.warning('', extra={'action': 'read_results', 'ip': '-', 'score': '-', 'rule': '-', 'status': 'warning', 'message': 'No results received from Splunk'})
            return

        logger.info('', extra={'action': 'read_results', 'ip': '-', 'score': '-', 'rule': '-', 'status': 'success', 'message': f'Received {len(results)} correlation detection(s) from Splunk'})

        # Process correlation results and trigger auto-blocking
        process_correlation_results(results)

        logger.info('', extra={'action': 'processing_complete', 'ip': '-', 'score': '-', 'rule': '-', 'status': 'success', 'message': 'Correlation result processing completed'})

    except Exception as e:
        logger.error('', extra={'action': 'main', 'ip': '-', 'score': '-', 'rule': '-', 'status': 'error', 'message': str(e)})
        sys.exit(1)
